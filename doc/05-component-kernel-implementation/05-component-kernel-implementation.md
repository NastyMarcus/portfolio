...
For the implementation of the Playlist component, I have chosen an ArrayList<Song> as the underlying data structure to represent the playlist. Because access elements by index is efficient in an ArrayList, which allows easy sorting, shuffling, and retrieving elements.

Convention:
Each element (song) in the ArrayList<Song> should be non-null to avoid NullPointerException during playback or other operations.
The order of songs should remain consistent unless altered intentionally by sorting or shuffling methods.

Correspondence:
The ArrayList<Song> corresponds to the playlist, where each element represents a Song in the playlist.
The index of each Song in the ArrayList represents the sequential order in which the songs will be played.
Operations on the playlist, such as adding or removing songs, directly modify the ArrayList, updating the abstract state of the playlist accordingly.
